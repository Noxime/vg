...
impl<T> Interpreter<T> {
    ...
    pub fn instr(
        ...
        match *instr {
            ...
            Block(ref result_type, ref instrs) => {
                self.block(t, sframe, result_type, instrs, funcs, tables, globals, mems)
            }
            Loop(_, ref instrs) => self.loop_(t, sframe, instrs, funcs, tables, globals, mems),
            ...
            Call(idx) => {
                ...
                self.call(t, f_addr, sframe, funcs, tables, globals, mems)
            }
            CallIndirect(idx) => {
    ...
    fn block(
        ...
        for instr in instrs {
            match self.instr(t, sframe, instr, funcs, tables, globals, mems)? {
    ...
    fn loop_(
        ...
        'outer: loop {
            for instr in instrs {
                let res = self.instr(t, sframe, instr, funcs, tables, globals, mems)?;
    ...
    fn type_iunary<O>(&self, v: O, op: &IUnOp) -> O
    where
        O: IntOp,
    {
        match *op {
    ...
    fn type_funary<O>(&self, v: O, op: &FUnOp) -> O
    where
        O: FloatOp,
    {
        match *op {
    ...
    fn type_ibin<O>(&self, c1: O, c2: O, op: &IBinOp) -> Option<O>
    where
        O: IntOp,
    {
        let res = match *op {
    ...
    fn type_fbin<O>(&self, c1: O, c2: O, op: &FBinOp) -> O
    where
        O: FloatOp,
    {
        match *op {
    ...
    fn type_itest<O>(&self, v: O, op: &ITestOp) -> bool
    where
        O: IntOp,
    {
        match *op {
    ...
    fn type_irel<O>(&self, c1: O, c2: O, op: &IRelOp) -> bool
    where
        O: IntOp,
    {
        match *op {
    ...
    fn type_frel<O>(&self, c1: O, c2: O, op: &FRelOp) -> bool
    where
        O: FloatOp,
    {
        match *op {
    ...
    fn call_host(
        &mut self,
        t: &mut T,
        f_inst: &HostFuncInst<T>,
        _sframe: &StackFrame,
        _funcs: &FuncInstStore<T>,
        ...
        if let Some(err) = (f_inst.hostcode)(
            t,
            &mut mems.0[0].data,
            &self.stack[args_start..],
            &mut self.return_buffer,
        ) {
            return Err(Trap {
    ...
    pub fn call(
        ...
        match funcs[f_addr] {
            FuncInst::Module(ref f_inst) => {
                self.call_module(t, f_inst, sframe, funcs, tables, globals, mems)?
            }
            FuncInst::Host(ref f_inst) => {
                self.call_host(t, f_inst, sframe, funcs, tables, globals, mems)?
            }
        };
    ...
    fn call_indirect(
        ...
        self.call(t, func_addr, sframe, funcs, tables, globals, mems)
    }
