...
pub fn type_func<T>(store: &Store<T>, funcaddr: FuncAddr) -> types::Func {
...
/// Allocate a table
pub fn alloc_table<T>(store: &mut Store<T>, tabletype: &types::Table) -> TableAddr {
    store.tables.alloc(&mut store.types_map, tabletype)
}

/// Get the type of a table
pub fn type_table<T>(store: &Store<T>, tableaddr: TableAddr) -> types::Table {
...
pub fn size_table<T>(store: &Store<T>, tableaddr: TableAddr) -> usize {
    assert!(store.tables.contains(tableaddr));
    store.tables[tableaddr].elem.len()
}

/// Grow a table by new elements
pub fn grow_table<T>(store: &mut Store<T>, tableaddr: TableAddr, new: usize) -> Option<Error> {
    assert!(store.tables.contains(tableaddr));
...
/// Allocate a memory
pub fn alloc_mem<T>(store: &mut Store<T>, memtype: &types::Memory) -> MemAddr {
    store.mems.alloc(&mut store.types_map, memtype)
}

/// Get the type of a memory
pub fn type_mem<T>(store: &Store<T>, memaddr: MemAddr) -> types::Memory {
...
pub fn read_mem<T>(store: &Store<T>, memaddr: MemAddr, addr: usize) -> Result<u8, Error> {
    assert!(store.mems.contains(memaddr));
...
pub fn size_mem<T>(store: &Store<T>, memaddr: MemAddr) -> usize {
    assert!(store.mems.contains(memaddr));
    store.mems.size(memaddr)
}

/// Grow a memory by new pages
pub fn grow_mem<T>(store: &mut Store<T>, memaddr: MemAddr, new: usize) -> Option<Error> {
    assert!(store.mems.contains(memaddr));
...
pub fn type_global<T>(store: &Store<T>, globaladdr: GlobalAddr) -> types::Global {
...
/// Read a global
pub fn read_global<T>(store: &Store<T>, globaladdr: GlobalAddr) -> values::Value {
    assert!(store.globals.contains(globaladdr));
...
fn allocate_and_init_module<T>(
    ...
    if let Some(idx) = module.start {
        ...
        invoke_func(t, store, func_addr, Vec::new())?;
    }
