...
impl Input {
    ...
    pub fn handle(&mut self, e: &Event) {
        match e {
            Event::Keyboard(e) => {
                self.kb_last = true;
                match e {
                    keyboard::Event::Up(k) => self.kb_cache[*k as usize] = Digital::Released,
                    keyboard::Event::Down(k) => self.kb_cache[*k as usize] = Digital::Pressed,
                    _ => (),
                }
            }
            Event::Mouse(e) => match e {
                mouse::Event::Moved(c) => self.m_pos = *c,
                mouse::Event::Up(b) => self.m_cache[*b as usize] = Digital::Released,
                mouse::Event::Down(b) => self.m_cache[*b as usize] = Digital::Pressed,
            ...
            Event::Gamepad { id, ev } => {
                ...
                if let Some(v) = self.gp_list.get_mut(id) {
                    match ev {
                        ...
                        gamepad::Event::Axis(axis, value) => v.1.axes[*axis as usize] = *value,
    ...
    pub fn main(&self) -> GamepadState {
        ...
        for (button, keys) in buttons.iter() {
            state.buttons[*button as usize] = combined(keys.iter().map(|k| self.key(*k)));
        }
